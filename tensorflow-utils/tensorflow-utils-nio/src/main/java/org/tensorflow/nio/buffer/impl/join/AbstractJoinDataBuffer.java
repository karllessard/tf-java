...
abstract class AbstractJoinDataBuffer<T, B extends DataBuffer<T>> extends AbstractDataBuffer<T, B> {
  ...
  public B duplicate() {
    ...
    AbstractJoinDataBuffer<T, B> duplicate = instantiate(duplicateBuffers, readOnly);
    duplicate.limit = limit;
    duplicate.currentBufferIdx = currentBufferIdx;
    return (B)duplicate;
  }

  abstract AbstractJoinDataBuffer<T, B> instantiate(B[] buffers, boolean readOnly);
  ...
  AbstractJoinDataBuffer(B[] buffers, boolean readOnly) {
    if (buffers.length == 0) {
