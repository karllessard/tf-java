 ...
 *
 * <p>Values of this type are represented as a sequence of bytes, preceded by some metadata such as
 * their length and their offset within the tensor.
 */
public interface TString extends NdArray<String>, TType {
   ...
   *
   * <pre>{@code
   * Tensor<TString> tensor = TString.copyOf(Charsets.UTF_16, vectorOfStrings);
   * String str = tensor.data().use(Charsets.UTF_16).get(0);  // returns first string of the tensor
   * }</pre>
   *
   ...
   *
   * <pre>{@code
   * byte[] bytes = tensor.data().asBytes().get(0);  // returns first sequence of bytes in the tensor
   * }</pre>
   *
   * @param src the source array giving the shape and data to the new tensor
   * @return the new tensor
...
class TStringImpl extends DenseNdArray<String> implements TString {
  ...
  static <T> Tensor<TString> createTensor(NdArray<T> src, Function<T, byte[]> toBytes) {
    long size = StringTensorBuffer.computeSize(src, toBytes);
    return Tensor.allocate(TString.DTYPE, src.shape(), size, data ->
        ((TStringImpl)data).tensorBuffer.init(src, toBytes)
    );
  }
